...
{
    public class PathFinding<T, TKey> where TKey : notnull
    {
        ...
        public PathFinding(IEnumerable<T> items, Func<T, Node<T>> getNode, Func<T, TKey> getKey, Func<T, IEnumerable<Neighbor<TKey>>> getNeighborKeys)
        {
        ...
        private Dictionary<TKey, Node<T>> GenerateNodes()
        {
            var dict = _items
                       .Select(_getNode)
        ...
        private static void FindPath(Node<T> startNode, Node<T> endNode)
        {
            ...
            do
            {
                ...
                foreach (var neighbor in node.Neighbors.OrderBy(n => n.Cost))
                {
                    ...
                    openNodes.Enqueue(neighbor.Node, costToStart);
                }
